...
<body class="admin-body">
	...
	<div class="container" style="max-width: 1200px; margin-top: 50px;">
		<div class="box" style="padding: 40px;">
			...
			<hr
				style="margin: 20px 0; border: 0; border-top: 1px solid rgba(255, 255, 255, 0.1);">
			...
			<form action="<c:url value='/consultaCandidatos'/>" method="get"
				class="search-form"
				style="display: flex; gap: 10px; margin-bottom: 30px; align-items: flex-end;">
				<div class="input-group" style="flex: 1; margin-bottom: 0;">
					<label>Filtrar por:</label> <select name="acao"
						style="width: 100%; padding: 10px; border-radius: 5px; background: rgba(255, 255, 255, 0.9);">
				...
				<div class="input-group" style="flex: 2; margin-bottom: 0;">
					<label>Valor da busca:</label> <input type="text" name="valor"
						placeholder="Digite o termo de busca..." required
						style="width: 100%; padding: 10px;">
				</div>

				<button type="submit" class="btn"
					style="padding: 10px 25px; height: 42px;">Buscar</button>
			</form>

			<div class="table-responsive" style="overflow-x: auto;">
				<table class="table-custom"
					style="width: 100%; border-collapse: collapse; color: white;">
					<thead>
						<tr
							style="background: rgba(255, 255, 255, 0.1); text-align: left;">
					...
					<tbody>
						<c:forEach var="c" items="${lista}">
							<tr style="border-bottom: 1px solid rgba(255, 255, 255, 0.05);">
								<td style="padding: 12px 15px;">${c.nome}</td>
								<td style="padding: 12px 15px;">${c.email}</td>
								<td style="padding: 12px 15px;"><span class="badge"
									style="background: rgba(255, 255, 255, 0.2); padding: 2px 8px; border-radius: 4px; font-size: 0.8rem;">
										${c.curso.nome	} </span></td>
								<td style="padding: 12px 15px;">${c.bairro}</td>
								<td style="padding: 12px 15px; font-size: 0.85rem; color: #ccc;">
									<fmt:parseDate value="${c.dataCad}"
						...
						<c:if test="${empty lista}">
							<tr>
								<td colspan="5"
									style="padding: 30px; text-align: center; color: #999;">Nenhum
									candidato encontrado.</td>
							</tr>
						</c:if>
					</tbody>
				</table>
